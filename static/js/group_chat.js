...
socket.onmessage = function (e) {
    ...
    if (data.username == message_username) {
        ...
        document.querySelector('#chat-body').innerHTML += `<tr>
                                                                <td>
                                                                <p class="bg-primary p-2 mt-2 mr-5 shadow-sm text-white float-left rounded">${data.message}<small style="font-size:0.5em; ">&nbsp; ${data.username}</small><small style="font-size:0.5em; ">&nbsp; ${d.toTimeString().slice(0, 5)}</small></p>
                                                                </td>
                                                            </tr>`
    }
}
